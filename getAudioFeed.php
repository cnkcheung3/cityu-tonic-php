...
function get_audio_feed($body, $conn){
    ...
    while($row = $result->fetch_assoc()){
        ...
        if($need_detail){
            ...
            $temp['like'] = getLikeCount($temp['fid'], $conn);
            $temp['liked'] = isLikeByMe($temp['fid'], $id, $conn);
        }else if($need_update){
            $temp['ut'] = $updated_time;
            $temp['like'] = getLikeCount($temp['fid'], $conn);
            $temp['liked'] = isLikeByMe($temp['fid'], $id, $conn);
...
function getLikeCount($fid, $conn){
    $sql = "SELECT * FROM Likes WHERE feed_id = '$fid'";
    $result = $conn->query($sql);
    return $result->num_rows;
}

function isLikeByMe($fid, $id, $conn){
